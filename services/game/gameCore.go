...
func(mage *Mage) GetOwner() string{
	owner:=mage.owner
	return owner
}
func(war *Warrior) GetOwner() string{
	owner:=war.owner
	return owner
}
func(rog *Rogue) GetOwner() string{
	owner:=rog.owner
	return owner
}
...
func(war *Warrior) SetAttacksPool() {
	war.attackpool = append(war.attackpool, AttacksPool{"swordPunch",7,7})
	war.attackpool = append(war.attackpool, AttacksPool{"lunge",6,6})
	war.attackpool = append(war.attackpool, AttacksPool{"rock",2,0})
}
func(mage *Mage) SetAttacksPool() {
...
func(war *Warrior) SetOwner(owner string) {
	war.owner = owner
}
func(mage*Mage) SetOwner(owner string) {
	mage.owner = owner
}
func(rogue *Rogue) SetOwner(owner string) {
	rogue.owner = owner
}
...
func createWarrior(owner string) *Warrior {
	warrior := Warrior{}
	warrior.unitType = "Warrior"
	warrior.owner = owner
	return setWarrior(&warrior)
}
func createMage(owner string) *Mage {
	mage := Mage{}
	mage.unitType = "Mage"
	mage.owner = owner
	return setMage(&mage)
}
func createRogue(owner string) *Rogue {
	rogue := Rogue{}
...
func (w *Warrior) printInfo(){
	fmt.Printf(" : %s , %s have %.2f hp, %.2f stamina,%.2f mana and %.2f def %s owner \n",w.name,w.unitType,w.health,w.stamina,w.mana,w.defence,w.owner)
}
func (m *Mage) printInfo(){
	fmt.Printf(" : %s , %s have %.2f hp, %.2f stamina,%.2f mana and %.2f def %s owner \n",m.name,m.unitType,m.health,m.stamina,m.mana,m.defence,m.owner)
}
func (r *Rogue) printInfo(){
	fmt.Printf(" : %s , %s have %.2f hp, %.2f stamina,%.2f mana and %.2f def %s owner \n",r.name,r.unitType,r.health,r.stamina,r.mana,r.defence,r.owner)
}

func (w *Warrior) PrintFullInfo(wr http.ResponseWriter){
	fmt.Fprintf(wr," : %s , %s have %.2f hp, %.2f stamina,%.2f mana and %.2f def %s owner \n",w.name,w.unitType,w.health,w.stamina,w.mana,w.defence,w.owner)
}
func (m *Mage) PrintFullInfo(w http.ResponseWriter){
	fmt.Fprintf(w," : %s , %s have %.2f hp, %.2f stamina,%.2f mana and %.2f def %s owner \n",m.name,m.unitType,m.health,m.stamina,m.mana,m.defence,m.owner)
}
...
func CreateRandomOne(owner string) Hero {
	num := rand.Intn(3)
	var hero Hero
	switch num{
	case 0:
		temp:= createWarrior(owner)
		hero = temp
	case 1:
		temp := createMage(owner)
		hero = temp
	case 2:
		temp := createRogue(owner)
		hero = temp
	}
	return hero
}
func StartBattleDB(heroes []Hero) (Hero,int) {
	if(!checkForEven(len(heroes))){
		print("Error! Not even count of players")
		return nil,0
	}
	message := make(chan Hero)
//	heroes := make([]Hero,len(heroes))
	tempheroes := make([]Hero,len(heroes))
	rand.Seed(time.Now().UnixNano())
	startLen := len(heroes)
	println("Players : ")
	for i:= range(heroes){
...
func StartBattle(count int) Hero {
	...
	for i := range heroes{
		...
		switch num{
		case 0:
			temp:= createWarrior("Admin")
			heroes[i] = temp
		case 1:
			temp:= createMage("Admin")
			heroes[i] = temp
		case 2:
			temp:= createRogue("Admin")
			heroes[i] = temp
		}
	}
	println("Players : ")
